$(document).ready(function(){
    ...
    var winResize = function(){
        ...
        if(winW <= 679){
            ...
            /* mininav onepage scroll */
            var onePageM = function(){
                var gnb_a = $(".mos_minav").find('a');
                gnb_a.on("click",function(e){
                    e.preventDefault();
                    var a_hash = $(this.hash),
                        offset_top = a_hash.offset().top,
                        t_head = $('header').height() +  $('#under_bar').height() + $('#nav_box').height(),
                        bodyBox = $("html,body");
                bodyBox.animate({scrollTop:offset_top - t_head},1500);
                });
            };
            ...
            /* mininav onepage scroll */
            var onePageT = function(){
                var gnb_a = $(".tab_minav").find('a');
                gnb_a.on("click",function(e){
                    e.preventDefault();
                    var a_hash = $(this.hash),
                        offset_top = a_hash.offset().top,
...
//                $('h1').stop().fadeIn(300);
            ...
            /* mininav onepage scroll */
            var onePageP = function(){
                var gnb_a = $(".pc_minav").find('a');
                gnb_a.on("click",function(e){
                    e.preventDefault();
                    var a_hash = $(this.hash),
                        offset_top = a_hash.offset().top,
                        bodyBox = $("html,body");
                bodyBox.animate({scrollTop:offset_top -220},1500);
                });
            };
